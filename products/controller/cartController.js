...
//             res.send('Product not found');
//         }
//         const bidFilePath = path.join(__dirname, 'bidding.json');
//         fs.readFile(bidFilePath, 'utf8', (err, data) => {
//             if (err) {
//                 console.error(err);
//                 return res.status(500).send('Unable to load cart');
//             }
//             let bid = JSON.parse(data);
//             const bidItem = bid.find(item => item.id === id);
//             if (!bidItem) {
//                 bid.push({ ...product, quantity: 1 });
//             }
//             fs.writeFile(bidFilePath, JSON.stringify(bid), (err) => {
//                 if (err) {
//                     console.error(err);
//                     res.send('Unable to update cart');
//                 }
//                 res.redirect('/biddingResult');
//             });
//         });
//     }
//     else if(process.env.STORE==='DB'){
...
//             }
//             // Check if this product already has a bidding record for this user
//             let bidProduct = await biddingproducts.findOne({
//                 user: userId, 
//                 name: product.name, 
//             });
//             if (!bidProduct) {
//                 bidProduct = new biddingproducts({
//                     user: userId, 
//                     name: product.name, 
//                     bidding, 
//                 });
...
// exports.bidResult = async(req,res)=>{
//         try {
...
//                 return res.send('No products to process.');
//             }
    
//             let ordersCreated = 0;
//             for (const prod of productsToProcess) {
//                 const highestBid = await biddingproducts.findOne({ name: prod.name })
//                     .sort('-bidding')
//                     .limit(1);
...
//             console.log(`Highest bid for product ${prod.name}:`, highestBid);

//         } catch (error) {
//             console.error('Error processing bid results:', error);
//             res.status(500).send('Error processing bid results.');
//         }
//     };
// }
// }
